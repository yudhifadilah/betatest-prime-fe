...
export default function DashboardVilogPage() {
  const [products, setProducts] = useState<VilogProduk[]>([]);
  const [orders, setOrders] = useState<VilogOrder[]>([]);
  ...
  const toggleSecret = (orderId: number) => {
  ...
  const parseBackupCodes = (backupCode?: string) => {
    if (!backupCode) return [];

    try {
      const parsed = JSON.parse(backupCode);
  ...
  const getOrders = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/vilog/orders`, {
        method: "GET",
        cache: "no-store",
        headers: {
          Accept: "application/json",
      ...
      if (!response.ok) {
        throw new Error(result.message || "Gagal mengambil order vilog");
      }
  ...
  const submitProduct = async () => {
    ...
    try {
      ...
      const url = editingId
        ? `${API_URL}/api/vilog/produk/${editingId}`
        : `${API_URL}/api/vilog/produk`;
      ...
      if (!response.ok) {
        throw new Error(result.message || "Gagal menyimpan produk vilog");
      }
  ...
  const editProduct = (item: VilogProduk) => {
  ...
  const deleteProduct = async (id: number) => {
    const confirmDelete = confirm("Yakin ingin menghapus produk vilog ini?");
  ...
  const updateOrderStatus = async (id: number, status: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/vilog/orders/${id}/status`, {
        method: "PATCH",
        headers: authHeaders,
  ...
  const submitCompletedStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/vilog/orders/${selectedCompleteOrder.id}/status`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${getToken()}`,
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((item) => {
      const produk = products.find(
        (p) => String(p.id) === String(item.vilogProdukId)
      );
      ...
      return (
        ...
        String(item.VilogProduk?.namaProduk || produk?.namaProduk || "")
          .toLowerCase()
          .includes(keyword)
      );
    });
  ...
  const getOrderProduct = (order: VilogOrder) => {
    const produkFromList = products.find(
      (item) => String(item.id) === String(order.vilogProdukId)
    );

    return {
      namaProduk:
        order.VilogProduk?.namaProduk || produkFromList?.namaProduk || "Vilog",
      harga: Number(order.VilogProduk?.harga || produkFromList?.harga || 0),
    };
  };
      ...
      <div className="mx-auto w-full max-w-7xl space-y-8">
        ...
        <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            subtitle="Produk vilog tersedia"
            icon={<Package size={24} />}
          />
          <StatCard
            title="Total Order"
            value={orders.length}
            subtitle="Order vilog masuk"
        ...
        <section className="rounded-[34px] border border-neutral-200 bg-white p-6 shadow-sm">
          ...
          {loadingOrders ? (
            ...
            <EmptyState text="Order vilog belum tersedia." />
          ) : (
                  ...
                  >
                    ...
                    <div className="mt-5 rounded-[24px] border border-neutral-200 bg-white p-4">
                      ...
                      <div className="mt-4 grid gap-4 md:grid-cols-2">
                        ...
                        <BackupCodeBlock
                          codes={parseBackupCodes(order.backupCode)}
                          visible={Boolean(visibleSecrets[order.id])}
                        />
                      </div>
                    </div>
